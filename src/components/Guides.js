...
class Guides extends React.Component {
  ...
  fetchMore = (
    ...
    const url =
      ...
      "solr/sharepoint_index/select?fl=file,url,rating,disclosure_level,isgoldcollateral,title,id,doc_source,modified_date,file_type,isarchived&fq=document_type:%22Guides%22%20AND%20document_type_details:%22" +
      this.state.popupHeader +
      "%22&indent=on&q=" +
      this.state.searchKey +
      "&rows=10&start=" +
      begin +
      "&wt=json" + (this.state.filters.indexOf("&fq=nda:\"True\"") === -1 ? "&fq=-nda:%22True%22" + URLConfig.GetUserRoles() : "") +
      this.state.filters +
  ...
  ShowMore = (categoryName) => {
    const URLGuides =
      URLConfig.get_APIHost() +
      "solr/sharepoint_index/select?fl=file,url,rating,disclosure_level,title,isgoldcollateral,id,doc_source,modified_date,file_type,isarchived&fq=document_type:%22Guides%22%20AND%20document_type_details:%22" +
      categoryName +
      "%22&indent=on&q=" +
      this.state.searchKey +
        ...
        {this.state.initialList !== undefined &&
          this.state.initialList.length > 0 && (
            <>
              <div
                className={
                  "card-header" + (this.props.isGuides ? " catgeorybg" : "")
                }
              >
