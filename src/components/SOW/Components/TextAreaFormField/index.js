...
const TextAreaFormField = ({
  ...
  useEffect(() => {
    ...
    if (value && value !== previousDynamicFieldValueRef.current) {
      ...
      const contentBlock = htmlToDraft(value ?? "<p><br/></p>");
      const contentState = ContentState.createFromBlockArray(
        contentBlock.contentBlocks,
        contentBlock.entityMap
      );
      const newState1 = EditorState.createWithContent(contentState);
  ...
  const mapKeyToEditorCommand = useCallback(
    (e) => {
      // console.log("mapKeyToEditorCommand", { keyCode: e.keyCode });
      switch (e.keyCode) {
        case 9: // TAB
          ...
          // console.log(newEditorState !== editorState);
