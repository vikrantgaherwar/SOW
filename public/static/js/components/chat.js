...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      $(BotResponse).appendTo(".chats").hide().fadeIn(200);
      scrollToBottomOfResults();
    } else {
...
function send(message) {
  //const sender_id = getCookie("empnumber");
  const sender_id = getCookie("name").split("%20")[0];
  //const sender_email =  getCookie("mail") + "_";
  $.ajax({
    url: rasa_server_url,
    type: "POST",
...
function customActionTrigger() {
    ...
    $.ajax({
        url: "https://hc4w00279.itcs.hpecorp.net:5055/webhook/",
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({
